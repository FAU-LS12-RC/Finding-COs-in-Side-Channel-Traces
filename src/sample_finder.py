...
class SampleFinder:
    def __init__(self, trace_container, top_x = 10, do_plots=False,print_info=True, error_margin=0.02,exact_clk_cycles = None,allowed_sub_peak_delta = 2):
        if(print_info):print("initialized the sample finder with " + str(trace_container.nr_hidden_cos) + " hidden aes cycles, sample rate of " + str(trace_container.get_fs()))
        
        self.trace_container = trace_container
        
        if exact_clk_cycles == None:    
            self.min_clk_cycles = max(1,int(trace_container.known_width_clk_cycles*0.95))
            self.max_clk_cycles = max(self.min_clk_cycles+1,int(trace_container.known_width_clk_cycles*1.05))
        else:
    ...
    def full_auto_find_COs(self,do_quality_plot=False, do_main_sub_peak_plot=False, sad_for_autocorr = False, sad_approach=True, avg_round_template=True, use_detrended = False):
        ...
        possible_widths =  np.arange(samples_per_clock*self.min_clk_cycles,samples_per_clock*self.max_clk_cycles,step=samples_per_clock,dtype=int)
        print("possible widths (samples): "+ str(possible_widths))
        print("possible widths (cycles):  "+ str(np.arange(self.min_clk_cycles,self.max_clk_cycles)))
        self.f_device = f_device

        #2. Test all possible widths, find starting index that has best autocorreltaion
        from time import process_time
        t1_start = process_time()  
        opencl_autocorr = Autocorrelation_Accelerator(self.trace_container.get_trace(),self.trace_container.no_similar_rounds,self.top_x,do_plots=do_quality_plot,use_detrended=use_detrended,trace_container = self.trace_container)
        if sad_for_autocorr:
            best_widths,widths_correlation,correlation_for_each_width = opencl_autocorr.autosad_accelerated_updated(possible_widths)
        else:
            best_widths,widths_correlation,correlation_for_each_width = opencl_autocorr.autocorrelation_accelerated_updated(possible_widths)
        t1_stop = process_time() 
        self.alg1_time_sec = t1_stop-t1_start
        if self.print_info:
            print("GPU: Finding best possible starting points used " + str((t1_stop-t1_start))+ " seconds")
            print(best_widths)
            best_width = possible_widths[int(best_widths[0,1])]
            starting_position = widths_correlation[int(best_widths[0,1]),0,1]
            #print(best_widths)
            print("best_width = "+ str(best_width) + " with correlation of: " + str(best_widths[0,0]) + " at starting position: "+ str(starting_position))
        ...
        best_fitting_width, peak_idx_list,char_trace_template = self.get_best_fitting_width(best_widths,possible_widths,widths_correlation,f_device, sad_approach=sad_approach, avg_round_template=avg_round_template, allowed_sub_peak_delta = self.allowed_sub_peak_delta, do_plots = do_main_sub_peak_plot)
        ...
        if self.print_info:
            ...
            print("found on cpu using " + str((find_best_fitting_width_end-find_best_fitting_width_start))+ " seconds")
            ...
            if delta > 1:
                for idx in np.arange(sub_peak_idx-delta,sub_peak_idx+delta,step=1,dtype=int):
                    if self.is_a_peak(x,idx,main_peak_idx,delta):
                        idx_is_peak = self.is_a_peak(x,idx,main_peak_idx,delta)
            if idx_is_peak:
                number_of_peaks+=1
            else:
                return number_of_peaks
    ...
    def find_peaks(self,correlation,w_segment,w_event, correlation_step_size,min_number_of_sub_peaks = 8,no_rounds=64,max_num_peaks=5000,allowed_sub_peak_delta = 2):
        if self.print_info:print("Finding main- and sub-peaks for width : " + str(w_segment*correlation_step_size))
        ...
        for i in range(max_num_peaks):
            ...
            if(max_idx > len(correlation_orig)-w_event): # add 0 if we need them!
                correlation = np.concatenate((correlation,np.zeros(w_event+1)))
                correlation_removed_peaks = np.concatenate((correlation_removed_peaks,np.zeros(w_event+1)))
                correlation_orig = np.concatenate((correlation_orig,np.zeros(w_event+1)))
            nr_of_peaks = self.number_of_equidistant_peaks(correlation_orig[max_idx:max_idx+w_event+2*allowed_sub_peak_delta],distance=w_segment,delta=allowed_sub_peak_delta,max_peaks=no_rounds)
            _print("nr_of_equidistant_peaks = "+ str(nr_of_peaks),print_info="debug")

            if True: #if we want to check before peaks
                nr_of_peaks_before = self.number_of_equidistant_peaks(np.flip(correlation_orig[np.max((max_idx-w_event-2*allowed_sub_peak_delta,0)):max_idx+1]),distance=w_segment,delta=allowed_sub_peak_delta,max_peaks=no_rounds)
                _print("nr_of_equidistant_peaks_before = "+ str(nr_of_peaks_before),print_info="debug")
            ...
            if nr_of_peaks<min_number_of_sub_peaks:
                ...
                if self.print_info:print("We found " + str(i) + " peaks. Not enough sub-peaks for next main-peak at position: " + str(max_idx*correlation_step_size))
                break
    ...
    def get_peaks_for_width(self,f_device,width, starting_position, min_number_of_sub_peaks=8, do_plots=False,print_info=False,max_num_peaks=5000,new_chosen_mean_event = None,sad_approach=False,avg_round_template=True,allowed_sub_peak_delta=1):
        ...
        if not w in self.correlation_dict:
            ...
            idx_list = np.arange(start_offset,len(self.trace_container.get_trace())-(self.trace_container.no_similar_rounds*w),step=correlation_step_size,dtype=int)
            #check bounds:
            if (i+w*self.trace_container.no_similar_rounds)>len(self.trace_container.get_trace()):
                return (-1,-1,-1)
            if avg_round_template:
                chosen_mean_segment = np.average([self.trace_container.get_trace()[i+w*j:i+w*(j+1)] for j in range(self.trace_container.no_similar_rounds)],axis=0)
                chosen_mean_event = np.array([chosen_mean_segment for j in range(self.trace_container.no_similar_rounds)]).flatten()
            else:
                chosen_mean_event = self.trace_container.get_trace()[i:i+w*self.trace_container.rounds_in_co_template]

            if not sad_approach:
                corrl_accl = Autocorrelation_Accelerator()                
                all_correlation = corrl_accl.correlate(self.trace_container.get_trace(), chosen_mean_event, idx_list, opencl=True)
                
                filtered_correlation = detrending_filter(all_correlation,(w)/correlation_step_size)
                ...
                corrl_accl = Autocorrelation_Accelerator()
                self.mean_event_dicts[w] = chosen_mean_event 
                sad_over_everything = np.array(corrl_accl.calc_sad(self.mean_event_dicts[w],self.trace_container.get_trace(),idx_list = idx_list))
        ...
        if(len(peak_idx_list) > self.trace_container.nr_hidden_cos):
                    ...
                    min_threshold = filtered_correlation[min_peak] - (filtered_correlation[max_peak] - filtered_correlation[min_peak])
    ...
    def get_best_fitting_width(self,best_widths,w_list,widths_correlation,f_device,sad_approach=False,avg_round_template=True,allowed_sub_peak_delta = 1, do_plots = False):
        ...
        least_peaks_array = np.zeros(len(best_widths[:,1]))+self.trace_container.no_similar_rounds
        peak_idx_list = []
        return_peak_idx_list = []
        
        chosen_mean_events = [np.zeros(int(best_widths[i,1])) for i in range(len(best_widths[:,1]))]
        for min_number_of_sub_peaks in np.arange(self.trace_container.no_similar_rounds-1,1,step=-1,dtype=int):
            if self.print_info:print("Try with min number of " + str(min_number_of_sub_peaks)+ " sub peaks for each CO!")
            print("--------------------------------------------------------------------")
            ...
            for width_idx,idx in zip(best_widths[:,1],range(len(best_widths[:,1]))):
                ...
                peak_idx_list,least_peaks_array[least_peaks_array_idx],chosen_mean_events[idx] = self.get_peaks_for_width(f_device,width,starting_position,min_number_of_sub_peaks = min_number_of_sub_peaks,do_plots=do_plots,max_num_peaks=(self.trace_container.nr_hidden_cos*2 + self.trace_container.nr_hidden_cos*allowed_sub_peak_delta),sad_approach=sad_approach,avg_round_template=avg_round_template,allowed_sub_peak_delta = allowed_sub_peak_delta)
                ...
                if(len(peak_idx_list) >= int(self.trace_container.nr_hidden_cos) and len(peak_idx_list) <= self.trace_container.nr_hidden_cos + int(self.trace_container.nr_hidden_cos*self.error_margin)  ): # Margin for error :)
                    ...
                    char_round_template = np.average([self.trace_container.get_trace()[first_index+width*j:first_index+width*(j+1)] for j in range(self.trace_container.no_similar_rounds)],axis=0)
                    char_trace_template = np.array([char_round_template for j in range(self.trace_container.no_similar_rounds)]).flatten()
    ...
    def find_COs_with_template(self,template,do_plots = False,print_info = True,use_sad = True, no_decimation = True):
        corrl_accl = Autocorrelation_Accelerator()
        samples_per_clock = self.trace_container.get_fs()/self.trace_container.calculated_device_frequency
        ...
        idx_list = np.arange(start_offset,len(self.trace_container.get_trace())-len(template),step=correlation_step_size,dtype=int)
        if use_sad:
            correlation = np.array(corrl_accl.calc_sad(template,self.trace_container.get_trace(),idx_list))*-1
        else:
            correlation = corrl_accl.correlate(self.trace_container.get_trace(), template, idx_list, opencl=True)
        filtered_correlation = detrending_filter(correlation,(int(self.trace_container.calculated_width))/correlation_step_size)
        if do_plots:
            curve = hv.Curve((range(0,len(filtered_correlation)),np.array(filtered_correlation)),label="filtered correlation")
