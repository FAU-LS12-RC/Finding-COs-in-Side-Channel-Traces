...
class Refiner:
    ...
    def get_template_with_corr(self, plot_template_on_index=0, plot_finished_template=False,use_top_x_percent=0,max_offset=None,print_info=False):
        ...
        for aes_idx,list_idx in zip(start_aes_list[1:],range(len(start_aes_list[1:]))):
            ...
            corr_values = accl.correlate(trace_segment,baseline_encryption,idx_list=range(max_offset*width*2),print_times=False)
            ...
            new_cut_trace = np.array([trace[int(aes_idx+best_offset-max_offset*width):int(aes_idx+len(baseline_encryption)+best_offset+max_offset*width)]])
            if use_top_x_percent > 0:
                df_id_to_sad.append({'list_idx':aes_idx,'sad_value':np.amax(corr_values)},ignore_index=True)
            #print(best_offset_width)
            if len(new_cut_trace[0])==0 or len(new_cut_trace[0])!=len(finished_cut_aes_traces[0]):
                continue
            finished_cut_aes_traces = np.append(finished_cut_aes_traces,new_cut_trace,axis=0)
        if plot_template_on_index>0:
            show_offset = 0
            nr_traces_to_show = plot_template_on_index
            reset_output()
