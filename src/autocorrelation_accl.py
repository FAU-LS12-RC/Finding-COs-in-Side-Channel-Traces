...
class Autocorrelation_Accelerator:
    ...
    def calc_sad(self, template_candidate,trace,idx_list=[]):
        ...
        trace_dev = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=trace_host)
        template_candidate_host = template_candidate.astype(np.float32)
        template_candidate_dev = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=template_candidate_host)
        correlation_dev = cl.Buffer(ctx, mf.WRITE_ONLY, trace_host.nbytes)
        template_length_dev = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.int32(len(template_candidate)))
        ...
        prg.correlate(queue, trace_host.shape, None, trace_dev, correlation_dev, template_candidate_dev, template_length_dev)
    ...
    def correlate(self,trace,template_candidate,idx_list,opencl=True,print_times = True):
        ...
        if not opencl:
            correlation = np.array([np.abs(scipy.stats.pearsonr(template_candidate,trace[idx:idx+len(template_candidate)])[0]) for idx in idx_list])
        else:
            #insert code here :)
            ctx = cl.create_some_context()
            queue = cl.CommandQueue(ctx)
        ...
        if print_times:
            if opencl:
                print("GPU: correlate used " + str((t1_stop-t1_start))+ " seconds")
            else:
                print("CPU: correlate used " + str((t1_stop-t1_start))+ " seconds")
        return correlation
    ...
    def autocorrelation_accelerated_updated(self,w_list):
        ...
        data_dev = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=data_host)

        no_similar_rounds_dev = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.int32(self.no_similar_rounds))
        ...
        for w in w_list: 
            ...
            prg.correlate(queue, data_host.shape, None, data_dev, correlation_dev, no_similar_rounds_dev)
            
            correlation_host = np.empty_like(data_host)
            cl.enqueue_copy(queue,correlation_host,correlation_dev)
            if self.use_detrended:
                correlation_host_detrended = detrending_filter(correlation_host,w*self.no_similar_rounds)
            if self.do_plots:
                if not self.use_detrended:
                    Plotter(range(len(correlation_host)),np.array(correlation_host,dtype=float),"Sample","Similarity","Similarity of width" + str(w) + " (autocorr)","Similarity (Step 1) for width: "+ str(w),decimation_factor=30)
                else:
                    Plotter(range(len(correlation_host_detrended)),np.array(correlation_host_detrended,dtype=float),"Sample","Similarity detrended","Correlation detrended of width " + str(w) + " (autocorr)","correlation_host_detrended  " + str(w),decimation_factor=30)
            
            self.trace_container.quality_plot = correlation_host
            
            if self.use_detrended:
                add_len = len(correlation_host) - len(correlation_host_detrended)
                correlation_host = np.concatenate((correlation_host_detrended,np.zeros(add_len)))
            
            correlation_for_each_width.append(correlation_host)
            top_x_correlation = top_x_array(np.array(correlation_host),self.top_x,scale=1)
    ...
    def autocorrelation_accelerated_knownWidth(self,w):
        #go through all possible widhts to determine the best one! (w = perfect segment width)
        w_list = np.array([w])
        best_widths, widths_correlation,correlation_for_each_width = self.autocorrelation_accelerated_updated(w_list)
    ...
    def autocorr_cpu(self,w_list):
        ...
        for w in w_list:   
            ...
            #all start positions need to be considered!
            correlation_list = autocorr_loop(len_all_rounds,w,self.data, self.no_similar_rounds)
    ...
    def autosad_accelerated_updated(self,w_list):
        ...
        for w in w_list: 
            ...
            correlation_host = np.empty_like(data_host)
            cl.enqueue_copy(queue,correlation_host,correlation_dev)
            correlation_host_detrended = detrending_filter(correlation_host[:len(correlation_host)-w*self.no_similar_rounds],w)

            

            if self.do_plots:    
                Plotter(range(len(correlation_host[:int(len(correlation_host)-w*self.no_similar_rounds)])),np.array(correlation_host[:int(len(correlation_host)-w*self.no_similar_rounds)],dtype=float),"Sample","Correlation","Correlation of right width (autocorr)","correlation_host",decimation_factor=10)
                Plotter(range(len(correlation_host_detrended)),np.array(correlation_host_detrended,dtype=float),"Sample","Correlation detrended","Correlation detrended of right width (autocorr)","correlation_host_detrended",decimation_factor=10)
            
            if self.use_detrended:
                add_len = len(correlation_host) - len(correlation_host_detrended)
                correlation_host = np.concatenate((correlation_host_detrended,np.zeros(add_len)))
            else:
                correlation_host = correlation_host / correlation_host.max(axis=0)
