...
from bokeh.io import output_notebook, reset_output, save, output_file
import numpy as np
...
class TraceContainer:
    ...
    def import_from_config(self, config, section, print_info=False):
        print("creating trace container from file " +
              str(section)+". \nImported Information:")

        self.sampling_frequency = self._check_and_get_int(
            config, section, 'fs')
        self.sampling_frequency_raw = self.sampling_frequency
        self.known_device_frequency = self._check_and_get_int(
            config, section, 'fd')
        self.no_similar_rounds = self._check_and_get_int(
            config, section, 'similar_rounds')
        self.rounds_in_co_template = self._check_and_get_int(
            config, section, 'rounds_in_co_template')
        self.nr_hidden_cos = self._check_and_get_int(
            config, section, 'nr_hidden_cos')
        self.known_width_clk_cycles = self._check_and_get_int(
            config, section, 'approx_width_cycles')
        self.voltage_scaling = self._check_and_get_int(
            config, section, 'voltage_scaling')
        self.voltage_offset = self._check_and_get_int(
            config, section, 'voltage_offset')
        self.N = self._check_and_get_int(config, section, 'N')

        if config.has_option(section, "known_start_idx"):
            self.known_start_idx_aes = json.loads(config.get(
                section, "known_start_idx"))  # TODO: TEST THIS!
        else:
            self.known_start_idx_aes = None
    ...
    def get_known_width(self, in_clk_cycles=False):
        ...
        if self.known_device_frequency != None and self.known_width_clk_cycles != None:
            self.known_width = self.get_fs()/self.known_device_frequency * \
                self.known_width_clk_cycles
    ...
    def get_samples_per_clk(self, use_calculated=False):
        if use_calculated:
            self.samples_per_clk = (int)(
                self.get_fs()/self.calculated_device_frequency)
        else:
            self.samples_per_clk = (int)(
                self.get_fs()/self.known_device_frequency)
    ...
    def resample(self, f_s_new, f_s_old=0):
        ...
        print("Resampling trace from "+str(f_s_old)+"S/s to "+str(f_s_new) +
              "S/s that is an integer multiple of the device frequency ("+str(self.known_device_frequency)+"Hz) . This can take a while...")

        old_len = len(self.get_trace(raw=True))
        new_len = int(f_s_new/f_s_old*old_len)
        f_interp = interp1d(range(old_len), self.get_trace(
            raw=True), kind='linear', fill_value=0)
        new_sample_points = [x*(f_s_old/f_s_new) for x in range(new_len-1)]
        new_sample_points.append(self.get_trace(
            raw=True)[len(self.get_trace(raw=True))-1])
        ...
        if self.get_trigger_trace() is not None:
            f_interp = interp1d(
                range(old_len), self.get_trigger_trace(), kind='linear', fill_value=0)
            new_sample_points = [x*(f_s_old/f_s_new) for x in range(new_len-1)]
    ...
    def decimate_trace(self, decimation_factor, do_plots=False):
        ...
        decimated_trace = signal.decimate(
            np.array(self.trace_raw), decimation_factor)
        ...
        print("Decimating signal by a factor of " +
              str(decimation_factor) + ". This can take a while.")

        self.trace = signal.decimate(
            np.array(self.trace_raw), decimation_factor)
        self.sampling_frequency = self.sampling_frequency_raw/decimation_factor
        if self.known_start_idx_aes != None:
            self.known_start_idx_aes = [
                int(idx/decimation_factor) for idx in self.known_start_idx_aes]
        self.decimation_factor = decimation_factor
        if not(self.trigger_trace_raw is None):
            self.trigger_trace = signal.decimate(
                np.array(self.trigger_trace_raw), decimation_factor)
    ...
    def plot_trace(self, show_idx=True, show_known_idx=True, decimate_for_plot_factor=10, save_filename=None, plot_range_min=0, plot_range_max=None, show_trigger_trace=True, max_found_idx_to_show=None, export_csv=False, title=None, x_axis_in_time=False):
        hv.extension('bokeh')
        output_notebook()
        p = figure(x_axis_label='Sample', y_axis_label='Amplitude',
                   plot_width=600, plot_height=300, title=None)
        #curve = hv.Curve((range(0,len(data)),np.array(data)),label="concat_trace")
        if plot_range_max == None:
            plot_range_max = len(self.get_trace())

        trace_to_plot = np.array(self.get_trace())[
        ...
        if decimate_for_plot_factor == 1:
            if not x_axis_in_time:
                ...
                x_data = np.arange(
                    0, len(trace_to_plot)/self.get_fs(), step=1/self.get_fs(), dtype=float)
                p.xaxis.axis_label = "Time [s]"
            # , legend_label="Power Trace")
            p.line(x=x_data, y=trace_to_plot, line_width=0.5)
            if show_trigger_trace == True and self.trigger_trace is not None:
                trigger_trace_to_plot = np.array(self.get_trigger_trace())[
                    plot_range_min:plot_range_max]
                p.line(x=x_data, y=trigger_trace_to_plot, line_width=0.5,
                       line_color='green', legend_label="Triggertrace")
            if show_idx == True and self.calculated_start_idx_aes is not None:
                for idx in self.calculated_start_idx_aes[:max_found_idx_to_show]:
                    vline = Span(location=idx-plot_range_min, dimension='height',
                                 line_color='red', line_dash='dotted', line_width=0.5)
                    p.add_layout(vline)
            if show_known_idx == True and self.known_start_idx_aes is not None:
                for idx in self.known_start_idx_aes[:max_found_idx_to_show]:
                    vline = Span(location=idx-plot_range_min, dimension='height',
                                 line_color='green', line_dash='dotted', line_width=0.5)
                    p.add_layout(vline)
            # hv.save(curve,'test.svg',fmt='',backend='bokeh')
        else:
            decimated_trace = signal.decimate(
                trace_to_plot, decimate_for_plot_factor)
            if not x_axis_in_time:
                x_data = range(0, len(decimated_trace))
            else:
                x_data = np.arange(
                    0, len(decimated_trace)/self.get_fs(), step=1/self.get_fs(), dtype=float)
                p.xaxis.axis_label = "Time [s]"
            # , legend_label="Power Trace")
            p.line(x=x_data, y=np.array(decimated_trace), line_width=0.5)
            if show_trigger_trace == True and self.trigger_trace is not None:
                trigger_trace_to_plot = np.array(self.get_trigger_trace())[
                    plot_range_min:plot_range_max]
                decimated_trigger_trace = signal.decimate(
                    trigger_trace_to_plot, decimate_for_plot_factor)
                p.line(x=x_data, y=np.array(decimated_trigger_trace),
                       line_width=0.5, line_color='green', legend_label="Triggertrace")
            if show_idx == True and self.calculated_start_idx_aes is not None:
                decimated_idx_aes = [
                    idx/decimate_for_plot_factor for idx in self.calculated_start_idx_aes[:max_found_idx_to_show]]
                for idx in decimated_idx_aes:
                    vline = Span(location=idx-int(plot_range_min/decimate_for_plot_factor),
                                 dimension='height', line_color='red', line_dash='dotted', line_width=0.5)
                    p.add_layout(vline)
            if show_known_idx == True and self.known_start_idx_aes is not None:
                decimated_idx_aes = [
                    idx/decimate_for_plot_factor for idx in self.known_start_idx_aes[:max_found_idx_to_show]]
                for idx in decimated_idx_aes:
                    vline = Span(location=idx-int(plot_range_min/decimate_for_plot_factor),
                                 dimension='height', line_color='green', line_dash='dotted', line_width=0.5)
                    p.add_layout(vline)
    ...
    def get_idx_from_trigger_trace(self, threshold=20, trigger_raise_delay=50, raw=False):
        ...
        print("Found " + str(len(known_start_idx)) +
              " start idx: \n" + str(known_start_idx))
    ...
    def eval_calc_vs_known(self):
        ...
        distance_old = np.array(
            [((sorted_old[i+1]-sorted_old[i])/self.get_known_width()) for i in range(len(sorted_old)-1)])
        print(distance_old)
        ...
        distance = np.array([((sorted_new[i+1]-sorted_new[i]) /
                            self.get_known_width()) for i in range(len(sorted_new)-1)])
        print(distance)
    ...
    def eval_calc_vs_refined(self, refined, do_plots=False):
        ...
        no_off = sorted_old-sorted_old[0]
        distance_old = np.array(
            [int((sorted_old[i+1]-sorted_old[i])/self.get_known_width()) for i in range(len(sorted_old)-1)])
        print(distance_old)
        ...
        print(sorted_new)
        no_off = sorted_new-sorted_new[0]
        distance = np.array([int((sorted_new[i+1]-sorted_new[i]) /
                            self.get_known_width()) for i in range(len(sorted_new)-1)])
        print(distance)
