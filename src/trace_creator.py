...
class TraceImporter:
    ...
    def decimate_trace(self,decimation_factor,do_plots=False):
        ...
        #decimate data:
        decimated_trace = signal.decimate(np.array(self.trace_container.trace_raw), decimation_factor)
        decimated_test_trace = signal.decimate(np.array(self.trace_container.trace_raw), decimation_factor)
        if do_plots:
            hv.extension('bokeh')
            reset_output()
            output_notebook()
            ...
            show(p)
        self.trace_container.sampling_frequency = self.trace_container.get_fs(raw=True)/decimation_factor
        ...
        if self.trace_container != None:
            self.trace_container.trace = signal.decimate(np.array(self.trace_container.trace_raw), decimation_factor)
            ...
            if self.trace_container.known_start_idx_aes !=None:
                self.trace_container.known_start_idx_aes = [int(idx/decimation_factor) for idx in self.trace_container.known_start_idx_aes]
            self.trace_container.decimation_factor = decimation_factor
            if not(self.trace_container.trigger_trace_raw is None):
                self.trace_container.trigger_trace = signal.decimate(np.array(self.trace_container.trigger_trace_raw), decimation_factor)
